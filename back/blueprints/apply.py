...
@APPLY.route("/apply", methods=["POST", "OPTIONS", "HEAD"])
def apply():
    """Apply route when user want to apply to a job
    ...
    if not token:
        ...
        man = Manager()
        man.applyUn(result)
        response = make_response({'message' : 'Its All good man'},200)
    ...
    try:
        ...
        result = request.get_json()
        man = Manager()
        man.applyCo(result, data['id'])
        response = make_response(jsonify({'message': "You applied for this job"}), 200)
        return cors( response)
